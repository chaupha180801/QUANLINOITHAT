...
export default function AddBill({ route, navigation}) {
  const {id, username} = route.params;
  const [dropdown, setDropdown] = useState(null);
  const [visible, setVisible] = React.useState(false);
  const [list_cus, setListCu] = useState([]);
  const [list_product, setListPro] = useState([]);
  let [choose_customer, setChooseCus] = useState(null);
      ...
      tx.executeSql(
        ...
        (tx, results) => {
          ...
          for (let i = 0; i < results.rows.length; ++i)
            temp.push({label: results.rows.item(i).TENKH, value: results.rows.item(i).MAKH});
          setListCu(temp);
        }
      );
    });
  ...
  const save_product  = () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        ...
        [choose_product],
        (tx, results) => {
          if(parseInt(results.rows.item(0).SOLUONG) >= parseInt(import_number)){
            ...
            price = results.rows.item(0).GIABAN;     
            setSL(soluong);
            setFilePath(filePath);
            setNameProduct(name_product);
            setPrice(price);
            list.push({masanpham: choose_product, tensp: name_product, giaban: price, soluongnhap: import_number, anh: filePath, soluongcu: soluong});
            console.log(list);
            setChoosePro(null);
            setImportNumber('');
            total_price();
            ...
            ToastAndroid.showWithGravity('Số lượng tồn sản phẩm không đủ để bán',
            ToastAndroid.SHORT,
            ToastAndroid.CENTER
            );
          }     
  ...
  const total_price = () =>{
    var sum = 0;
    for(var i = 0; i < list.length; i++){
      sum = sum + ( parseInt(list[i].soluongnhap) *  list[i].giaban);       
    }
  ...
  const add_bill = () => {
    if (!choose_customer) {
      ToastAndroid.showWithGravity('Vui lòng chọn khách hàng',
      ToastAndroid.SHORT,
      ToastAndroid.CENTER
      );
      return;
    ...
    db.transaction(function (tx) {
      tx.executeSql(
        'INSERT INTO HOADON (MANV, NGAYLAP, THANHTIEN, MAKH, SOLUONGSP) VALUES (?,?,?,?,?)',
        [id, ngay_nhap, total, choose_customer, list.length],      
        (tx, results) => {
          if (results.rowsAffected > 0) {
            ...
            for(var i = 0; i < list.length; i++){
              tx.executeSql(
                'INSERT INTO CHITIETHOADON (SOHD, MASP, TENSP, SOLUONG, GIABAN, ANH) VALUES (?,?,?,?,?,?)',
                [curr_id, list[i].masanpham, list[i].tensp, list[i].soluongnhap, list[i].giaban, JSON.stringify(list[i].anh)]);
                tx.executeSql('UPDATE SANPHAM SET SOLUONG = ? WHERE MASP = ?',
                [ parseInt(list[i].soluongcu) - (parseInt(list[i].soluongnhap)).toString(), list[i].masanpham]);
            }   
            Alert.alert(
              'Thành công',
              ...
              [
                {
                  text: 'Ok',
                  onPress: () => navigation.navigate('ListBill', {id: id,username: username}),
                },
              ],
              { cancelable: false }
            ); 
          } else alert('Thêm hóa đơn không thành công');
        }
      );
    });
  };
    ...
    <View>
      <ModalPoup visible={visible} >
          ...
          <View styles={styles.header}>
              ...
              <Text style={styles.text_header}>Thêm chi tiết hóa đơn</Text>
          </View>
          <Dropdown
            ...
            placeholder="Chọn nhà khách hàng"
            value={choose_customer}
            onChange={item => {
              setChooseCus(item.value);
              console.log('selected', item);
            }}
            selectedTextStyle={{
              color: "#000000",
          ...
          <TextInput
            ...
            />
          <TouchableOpacity style={styles.button5} onPress={add_bill}>
             <Text style={styles.text2}>Lưu</Text>
          </TouchableOpacity>
        </View>
  )
